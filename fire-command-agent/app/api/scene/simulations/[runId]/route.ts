...
export async function GET(_request: Request, context: { params: Promise<{ runId: string }> }) {
  ...
  try {
    const response = await fetch(`${endpoint}/simulations/${encodeURIComponent(runId)}`, {
      headers,
    ...
    return NextResponse.json({ message: '三维推演状态查询超时。' }, { status: 504, headers: noStore });
  }
}
